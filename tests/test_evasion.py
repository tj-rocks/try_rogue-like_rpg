...
def test_evasion_statistical():
    ...
    with patch.object(Player, 'block_chance_close', new_callable=PropertyMock) as mock_block:
        mock_block.return_value = 0.20 # 20%
    ...
    # 2. 回避率 30% のテスト
    with patch.object(Player, 'block_chance_close', new_callable=PropertyMock) as mock_block:
        mock_block.return_value = 0.30 # 30%
        ...
        print(f"回避率 30% テスト結果: Miss={miss_count}/{trials} ({combined_rate*100:.1f}%)")
        
        # 統計的な誤差を考慮 (25% ~ 35% なら合格)
        assert 0.25 <= combined_rate <= 0.35, f"回避率が異常です: {combined_rate*100:.1f}% (Target: 30%)"

    print("[SUCCESS] 回避率の統計テストが正常に完了しました。")
